						...
						if (evac_vehicleKey != "0") then {
							...
							if (evac_hasKey) then {
								...
								if (s_player_makeEvacChopper < 0) then {
									s_player_makeEvacChopper = player addAction ["Set Evac-Chopper","custom\JAEM\SetEvacChopper.sqf",evac_cTarget,-1000,false,false,"",""];
								};
								...
								if (playerHasEvacField) then {
									...
									if ((evac_cTarget distance playersEvacField) <= 10) then {
										...
										if (s_player_clearEvacChopper < 0) then {
											s_player_clearEvacChopper = player addAction ["Clear Evac-Chopper","custom\JAEM\ClearEvacChopper.sqf",evac_cTarget,-1001,false,false,"",""];
										};
					...
					if (vehicle player == player) then {
						...
						if ((player distance playersEvacField) >= evac_MinDistance) then {
							...
							if (evac_needRadio == 1) then {
								...
								if (s_player_evacCall < 0) then {
									s_player_evacCall = player addAction ["Call Evac-Chopper","custom\JAEM\CallEvacChopper.sqf",[],-1000,false,false,"",""];
								};
							};
