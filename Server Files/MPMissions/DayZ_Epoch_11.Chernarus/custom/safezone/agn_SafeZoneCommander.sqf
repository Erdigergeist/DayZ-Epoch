...
if ( AGN_safeZoneMessages ) then { 
  sleep 20;
  systemChat ("Safe Life Mod Loaded!");
  sleep 3;
  systemChat ("You are save near traders and in the Trader Cities.");
};
...
while {true} do {
	...
	if ( AGN_safeZoneMessages ) then { systemChat ("Entering Trader Area - Safe Life Mod Enabled"); };
	_thePlayer = player;
	...
	if ( AGN_safeZoneMessages ) then { systemChat ("Exiting Trader Area - Safe Life Mod Disabled"); };
