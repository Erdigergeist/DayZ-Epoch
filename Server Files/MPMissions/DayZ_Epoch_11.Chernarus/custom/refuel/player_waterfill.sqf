...
private["_Building","_vehicle","_inVehicle","_RainAmt","_HeavyRailFillTime","_MediumRainFillTime","_LightRainFillTime","_playerPos","_canFill","_isRain","_isPond","_isWell","_pondPos","_objectsWell","_onLadder","_hasbottleitem","_config","_item","_text","_objectsPond","_qty","_dis","_sfx"];

// Gather critical core variables
call gear_ui_init;
_item = _this;
...
_HeavyRainFillTime = 30; // Time in seconds to refill bottle in very heavy rain; default is 12
_MediumRainFillTime = 45; // Time in seconds to refill bottle in medium/moderate rain; default is 18
_LightRainFillTime = 60; // Time in seconds to refill bottle in light rain; default is 26
_CleanRainWater = false; // Acceptable values are "true" or "false"; "false" denotes collected rain water needs boiling with tin can

// Initial disqualifications
_onLadder = (getNumber (configFile >> "CfgMovesMaleSdr" >> "States" >> (animationState player) >> "onLadder")) == 1;
if (_onLadder) exitWith { cutText [format["You cannot fill a water bottle while on a ladder."], "PLAIN DOWN", 3];  };
if (!_hasbottleitem) exitWith {cutText [format[(localize "str_player_31"),_text,(localize "str_player_31_fill")] , "PLAIN DOWN", 3]};
_vehicle = vehicle player;
_inVehicle = (_vehicle != player);
if (_inVehicle) exitwith {cutText [format["You may not do this while in a vehicle."] , "PLAIN DOWN", 3]};
...
if (_canFill) then {
	...
	if ("ItemWaterbottleUnfilled" in magazines player) then {
		...
		if (_Qty > 1) then {
		  cutText [format["You have filled %1 water bottles. Please boil it.",_Qty], "PLAIN DOWN", 3];
		} else {
		  cutText [format["You have refilled your water bottle. Please boil it."], "PLAIN DOWN", 3];
		};
	} else {
		cutText [format["You have no empty water bottles to fill."], "PLAIN DOWN", 3];
	};
};

if (!_canFill && !_isRain) exitwith { // No water well or pond nearby and not raining?
	cutText [format["You must be near a water source (a pond or well) or it must be raining to refill water bottle(s)."], "PLAIN DOWN", 3];
};

// It's raining! Hallelujah! Let's find out how hard it's raining then start an interruptible loop
if (!_canFill && _isRain && ("ItemWaterbottleUnfilled" in magazines player)) then {
  
  // Rain water collecting disqualifications 
  //Are we swimming? We cannot be to fill a water bottle
  if (dayz_isSwimming) exitwith { cutText [format["You must not be swimming to refill your water bottle in the rain."], "PLAIN DOWN", 3]; };
  // We inside a building? referenced in \z\addons\dayz_code\compile\fn_temperatur.sqf
  if (dayz_inside) exitwith { cutText [format["You must be outside of a building or not under a tree to fill with rain water."], "PLAIN DOWN", 3]; };
  ...
  if (_RainAmt > 0.80) then { // heavy rain
	_RefillTime = _HeavyRainFillTime; };
  if (_RainAmt > 0.60) then { // medium rain
    _RefillTime = _MediumRainFillTime; }; 
  if (_RainAmt < 0.60) then { // light rain
    _RefillTime = _LightRainFillTime; }; 
  
  // Inform the player we're starting and how long it will take
  if (_Qty == 1) then {
    cutText [format["You begin filling the empty water bottle. This will take %1 seconds. Do not interrupt.",_RefillTime], "PLAIN DOWN", 3];
  } else {
    cutText [format["You begin filling %1 empty water bottles. This will take %2 seconds. Do not interrupt.",_Qty,_RefillTime], "PLAIN DOWN", 3];
  };
  ...
  // Loop thru required time to fill and check for interruptions
	while {r_doLoop} do { 
		if (!r_interrupt) then {
			...
			if (_RainAmt < 0.025) then {
				cutText [format["The rain has stopped. Water bottle filling has been interrupted."], "PLAIN DOWN", 3];
				r_doLoop = false;
				r_action = false;
				_interrupted = true;
			};
        ...
        if (r_interrupt) then {
			cutText [format["Water bottle filling interrupted."], "PLAIN DOWN", 3];
			r_doLoop = false;
			r_action = false;
			_interrupted = true;
        };    
	...
	if (!_interrupted) then {
	  if (_CleanRainWater) then {
		if (_Qty == 1) then {
			cutText [format["You've successfully filled your water bottle with safe to drink, clean rain water."], "PLAIN DOWN", 3];
		} else {
			cutText [format["You've successfully filled %1 water bottles with safe to drink, clean rain water.",_Qty], "PLAIN DOWN", 3]; 
		};
	} else {
	    if (_Qty == 1) then {
			cutText [format["You've successfully filled your water bottle with rain water. Boil to make it safe."], "PLAIN DOWN", 3];
		} else {
			cutText [format["You've successfully filled %1 water bottles with rain water. Boil to make it safe.",_Qty], "PLAIN DOWN", 3]; 
		};
	};
