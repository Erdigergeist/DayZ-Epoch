...
_isHemp = cursorTarget isKindOf "fiberplant";
_isPump = cursorTarget isKindOf "MAP_pumpkin";
_isSunf = cursorTarget isKindOf "MAP_p_Helianthus";
...
if (!isNull cursorTarget && !_inVehicle && !_isPZombie && (player distance cursorTarget < _allowedDistance) && _canDo) then {	//Has some kind of target
  ...
  _itemsPlayer = items player;
    ...
    //AI-Leiche verbrennen
    if (!_isAlive && _isZombie) then {
      if (s_player_bury_zombie < 0) then {
        s_player_bury_zombie = player addAction [format["Leiche verbrennen (-10 Humanity)"], "custom\bury_human\kill_human.sqf",cursorTarget, 3, true, true, "", ""];
        }
      } else {
        player removeAction s_player_bury_zombie;
          s_player_bury_zombie = -1;
		};

    //AI beerdigen
    if (!_isAlive && _isZombie) then {
      if (s_player_burn_zombie < 0) then {
        s_player_burn_zombie = player addAction [format["Infektion eindaemmen (+10 Humanity)"], "custom\bury_human\bury_zombie.sqf",cursorTarget, 4, false, true, "", ""];
        }
      } else {
        player removeAction s_player_burn_zombie;
          s_player_burn_zombie = -1;
		};
    
    //Leiche verbrennen
    if (!_isAlive && !_isZombie && !_isAnimal && _isMan) then {
      if (s_player_burn_human < 0) then {
        s_player_burn_human = player addAction [format["Leiche verbrennen (-10 Humanity)"], "custom\bury_human\kill_human.sqf",cursorTarget, 3, false, true, "", ""];
        }
      } else {
        player removeAction s_player_burn_human;
          s_player_burn_human = -1;
		};

    // beerdigen
    if (!_isAlive && !_isZombie && !_isAnimal && _isMan) then {
      if (s_player_bury_human < 0) then {
        s_player_bury_human = player addAction [format["Beerdigen (+10 Humanity)"], "custom\bury_human\bury_human.sqf",cursorTarget, 4, false, true, "", ""];
        }
      } else {
  ...
  if (_isHemp) then {
		if (s_player_hemp < 0) then {
			s_player_hemp = player addAction [format["Harvest Weed"], "custom\harvest\hemp.sqf" , _cursorTarget, 5, false, true, "",""];
		};
	} else {
	...
	//Harvest Pumpkin
  if (_isPump) then {
		if (s_player_pump < 0) then {
			s_player_pump = player addAction [format["Harvest Pumpkin"], "custom\harvest\pumkin.sqf", _cursorTarget, 5, false, true, "",""];
		};
	} else {
		player removeAction s_player_pump;
		s_player_pump = -1;
	};
	//Harvest Sunflower
  if (_isSunf) then {
		if (s_player_sunf < 0) then {
			s_player_sunf = player addAction [format["Harvest Sunflowerseeds"], "custom\harvest\sunflower.sqf", _cursorTarget, 5, false, true, "",""];
		};
	} else {
		player removeAction s_player_sunf;
		s_player_sunf = -1;
	};
  ...
  if (_isMan and !_isAlive and !_isZombie and !_clothesTaken) then {
      if (s_player_clothes < 0) then {
          s_player_clothes = player addAction [("Take Clothes"), "custom\ztc\player_takeClothes.sqf",[_cursorTarget], -10, false, true, "",""];
    };
  } else {
    ...
    if (s_player_repair_crtl < 0) then {
			...
			_menu = dayz_myCursorTarget addAction [localize "STR_EPOCH_PLAYER_REPAIRV", "custom\salvage\repair_vehicle.sqf",_cursorTarget, 0, true, false, "",""];
			s_player_repairActions set [count s_player_repairActions,_menu];
      if(!locked _cursorTarget) then {
			_menu1 = dayz_myCursorTarget addAction [localize "STR_EPOCH_PLAYER_SALVAGEV", "custom\salvage\salvage_vehicle.sqf",_cursorTarget, 0, true, false, "",""];      
...
if (_canDrink) then {
        if (s_player_drinkWater < 0) then {
            s_player_drinkWater = player addaction[("Drink water"),"custom\drink_water\drink_water.sqf"];
        };
    } else {
