...
if (isNil "ELE_RequiredBuildTools") then { ELE_RequiredBuildTools = ["ItemToolbox", "ItemCrowbar", "ItemSledge"] }; // required tools for building an elevator and elevator stop
if (isNil "ELE_RequiredBuildItems") then { ELE_RequiredBuildItems = [["PartGeneric",4],["ItemPole",2],"PartEngine", "ItemGenerator", "ItemJerrycan"] }; // required items to build an elevator
if (isNil "ELE_RequiredBuildStopItems") then { ELE_RequiredBuildStopItems = [["PartGeneric",3],["ItemPole",2],"ItemBriefcase100oz"] }; // required items to build an elevator stop
if (isNil "ELE_Debug") then { ELE_Debug = false }; // debug flag

ELE_elevator = nil;

// global functions
call compile preprocessFileLineNumbers "custom\elevator\vector.sqf";
call compile preprocessFileLineNumbers "custom\elevator\ac_functions.sqf";
call compile preprocessFileLineNumbers "custom\elevator\elevator_functions.sqf";
	...
	if ((!isNull _ct) && {(player distance _ct) < ELE_Size}) then {
		...
		if (typeOf _ct == ELE_PlatformClass) then {
			...
			if ([_ct] call ELE_fnc_isElevator) then {
				if (s_player_elevator_next < 0 && {[_ct] call ELE_fnc_hasNextStop}) then {
					s_player_elevator_next = player addAction ["Elevator: Next Stop", "custom\elevator\elevator_actions.sqf", ["next",_ct], 5, false];
				};
				if (s_player_elevator_previous < 0 && {[_ct] call ELE_fnc_hasPreviousStop}) then {
					s_player_elevator_previous = player addAction ["Elevator: Previous Stop", "custom\elevator\elevator_actions.sqf", ["previous",_ct], 5, false];
				};
				if (s_player_elevator_select < 0) then {
					s_player_elevator_select = player addAction ["Select Elevator", "custom\elevator\elevator_actions.sqf", ["select",_ct], 1, false];
				};
			} else {
				if (s_player_elevator_upgrade < 0) then {
					s_player_elevator_upgrade = player addAction ["Upgrade to Elevator", "custom\elevator\elevator_build.sqf", ["build",_ct], 0, false];
				};
				if (s_player_elevator_upgrade_stop < 0) then {
					s_player_elevator_upgrade_stop = player addAction ["Upgrade to Elevator Stop", "custom\elevator\elevator_build.sqf", ["build_stop",_ct], 0, false];
				};
			};
		};
		...
		if ([_ct] call ELE_fnc_isElevatorStop) then {
			if (s_player_elevator_call < 0) then {
				s_player_elevator_call = player addAction ["Call Elevator", "custom\elevator\elevator_actions.sqf", ["call",_ct], 5, false];
			};
		};
		// debug actions
		if (s_player_elevator_id < 0 && ELE_Debug) then {
			s_player_elevator_id = player addAction ["Show Elevator ID", "custom\elevator\elevator_actions.sqf", ["id",_ct], 0, false];
		};
	} else {
