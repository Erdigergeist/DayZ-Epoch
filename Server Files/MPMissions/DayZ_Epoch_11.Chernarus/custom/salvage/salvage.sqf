...
if(TradeInprogress) exitWith { cutText ["Salvage already in progress." , "PLAIN DOWN", 3]; };
...
if (_hasToolbox) then {
	...
	player playActionNow "Medic";
      _string2 = format["Removing selected parts - please hold on"]; //Remove - Part 
      cutText [_string2, "PLAIN DOWN", 3];
	...
	if (_finished) then {
		...
		if (_damage < 0.75) then {
			...
			if (_isOK) then {	
				...
				if(_brokenPart) then {
					//Failed!
          _FailedMessage = format["You have destroyed %1 while attempting to remove from %2",_namePart,_nameType];
          cutText [_FailedMessage, "PLAIN DOWN", 3];
				} else {
					//Success!
          _SuccessMessage = format["You have successfully removed %1 from the %2",_namePart,_nameType];
          cutText [_SuccessMessage, "PLAIN DOWN", 3];
				};
				...
				cutText [localize "STR_DAYZ_CODE_2", "PLAIN DOWN", 3];
			};
		...
		cutText ["Canceled Salvage.", "PLAIN DOWN", 3];
	};
			
} else {
  _needMessage = format["You need %1 to remove this part.",_namePart];
  cutText [_needMessage, "PLAIN DOWN", 3];
};
