...
if (dayz_combat == 1) exitwith { cutText [format["You are in combat and cannot modify your weapon!"], "PLAIN DOWN", 3]};
if (_disallowinVehicle && vehicle player != player) exitWith {cutText [format["You cannot modify your weapon in a vehicle!"], "PLAIN DOWN", 3]};

player playActionNow "Medic";
[player,"repair",0,false] call dayz_zombieSpeak;
cutText [ format ["Attempting to craft %1",_makeItemName],"PLAIN DOWN", 3];
sleep 5;
...
if (_correctInventory) then { // appears the player has everything needed
    // remove the items now
    {
        ...
        for "_i" from 0 to _itemQty2-1 do {
            if (_itemType2 == 0) then {
                cutText [format ["Hold on, removing Item(s)"], "PLAIN DOWN",1];
                player removeMagazine _itemClass2;
            } else {
                cutText [format ["Hold on, removing Weapon"], "PLAIN DOWN",1];
    ...
    player playActionNow "Medic";
    [player,"repair",0,false] call dayz_zombieSpeak;
    cutText [ format ["Crafting a %1",_makeItemName],"PLAIN DOWN", 3];
    sleep 6;
    ...
    if (alive player) then { // see if they weren't killed in the process
        ...
        if(_error) then {
            cutText ["One or more items were lost in the crafting process.", "PLAIN DOWN", 3];
        } else {
            cutText [ format ["You have successfully created: %1",_makeItemName],"PLAIN DOWN", 3];
        };
    };
} else {
    ...
    _neededString = [];
    {
        ...
        _neededString set [count _neededString, format["%2 %3",_inUserInv3,_itemQty3,_itemClass3]];
