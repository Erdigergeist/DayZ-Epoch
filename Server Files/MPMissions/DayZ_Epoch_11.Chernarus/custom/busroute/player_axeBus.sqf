...
do
{
	...
	
	{
		if(_x getVariable["isAxeAIBus",0]==1)then{	
			_getInEvent = _x addEventHandler ["GetIn", {_this execVM "custom\busroute\player_join_bus.sqf"}];
			_x setVariable ["axeGetInBusEv",_getInEvent,false];
			_getOutEvent = _x addEventHandler ["GetOut", {_this execVM "custom\busroute\player_leave_bus.sqf"}];
