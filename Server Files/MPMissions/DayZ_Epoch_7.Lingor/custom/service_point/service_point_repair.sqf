...
private ["_vehicle","_args","_servicePoint","_costs","_repairTime","_type","_name","_hitpoints","_allRepaired","_textMissing","_message"];
...
if !([[[_costs select 0, _costs select 1]],0] call epoch_returnChange) then {
	...
	_allRepaired = true;
	{
		...
		if (_damage > 0) then {
			if (_repairTime > 0) then {
				...
				titleText [format["Repairing%1... Wait %2 seconds.", _partName,_repairTime], "PLAIN DOWN",_repairTime];
				sleep _repairTime;
			};
	...
	if (_allRepaired) then {
		...
		titleText [format["Repairing %1 done, thanks for your visit, pls come back and have a nice day", _name], "PLAIN DOWN"];
	};
};
