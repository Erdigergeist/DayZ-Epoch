...
if(_sleepTime < _sleepCooldown) exitWith { // If cooldown is not done then exit script
	cutText [format["You are not tired yet please wait %1!",(_sleepTime - _sleepCooldown)], "PLAIN DOWN", 3]; //display text at bottom center of screen when players cooldown is not done
};

if (dayz_combat == 1) then { // Check if in combat (dunno why you would want to sleep in combat but the check is here anyway)
    cutText [format["You cannot sleep while in combat!"], "PLAIN DOWN", 3]; //display text at bottom center of screen when in combat
} else {
	...
	if (_timeLeft == 0 || _timeLeft < 0) then { // If sleep countdown is done then.....
		...
		cutText [format["You are awake and fully rested!"], "PLAIN DOWN", 3]; // Text displayed on wakeup!
