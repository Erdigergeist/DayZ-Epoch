...
_countFuel = _countFuel + (count ((position _currentVehicle) nearObjects ["Land_fuel_tank_stairs_ep1", _distance]));
...
_countFuel = _countFuel + (count ((position _currentVehicle) nearObjects ["Land_ibr_FuelStation_Feed", _distance]));
...
_vehicle_refuel_id = _vehicle addAction ["Refuel", "custom\refuel\kh_vehicle_refuel.sqf", [_amount], -1, false, true, "", "vehicle _this == _target && local _target"];
};
};
