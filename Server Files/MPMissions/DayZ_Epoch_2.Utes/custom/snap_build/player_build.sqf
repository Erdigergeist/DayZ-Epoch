...
*/
private ["_location","_dir","_classname","_item","_hasRequiredTools","_missingT","_missingB","_hastoolweapon","_cancel","_reason","_started","_finished","_animState","_isMedic","_dis","_sfx","_hasbuilditem","_tmpbuilt","_onLadder","_isWater","_require","_text","_offset","_IsNearPlot","_isOk","_location1","_location2","_counter","_limit","_proceed","_num_removed","_position","_object","_canBuildOnPlot","_friendlies","_nearestPole","_ownerID","_findNearestPoles","_findNearestPole","_distance","_classnametmp","_ghost","_isPole","_needText","_lockable","_zheightchanged","_rotate","_combination_1","_combination_2","_combination_3","_combination_4","_combination","_combination_1_Display","_combinationDisplay","_zheightdirection","_abort","_isNear","_need","_objHupDiff","_needNear","_vehicle","_inVehicle","_previewCounter","_requireplot","_objHDiff","_isLandFireDZ","_isTankTrap","_isNear2","_typeIsString","_isBuildAdmin","_needBuildItem","_hasbuilditems","_itemIn","_countIn","_qty","_missingQty","_textMissing","_removed","_tobe_removed_total","_removed_total"];

if(DZE_ActionInProgress) exitWith { titleText [(localize "str_epoch_player_40") , "PLAIN DOWN", 3]; };
DZE_ActionInProgress = true;

// disallow building if too many objects are found within 30m
if((count ((getPosATL player) nearObjects ["All",30])) >= DZE_BuildingLimit) exitWith {DZE_ActionInProgress = false; titleText [(localize "str_epoch_player_41"), "PLAIN DOWN", 3];};
...
if (_isWater) exitWith {DZE_ActionInProgress = false; titleText [localize "str_player_26", "PLAIN DOWN", 3];};
if (_inVehicle) exitWith {DZE_ActionInProgress = false; titleText [(localize "str_epoch_player_42"), "PLAIN DOWN", 3];};
if (_onLadder) exitWith {DZE_ActionInProgress = false; titleText [localize "str_player_21", "PLAIN DOWN", 3];};
if (player getVariable["combattimeout", 0] >= time) exitWith {DZE_ActionInProgress = false; titleText [(localize "str_epoch_player_43"), "PLAIN DOWN", 3];};
...
if (_typeIsString) then {
	...
	if(_abort) exitWith {
		titleText [format[(localize "str_epoch_player_135"),_reason,_distance], "PLAIN DOWN", 3];
...
if(_isPole and _IsNearPlot > 0) exitWith {  DZE_ActionInProgress = false; titleText [(localize "str_epoch_player_44") , "PLAIN DOWN", 3]; };
...
//Item Check
if(!_canBuildOnPlot) exitWith {  DZE_ActionInProgress = false; titleText [format[(localize "STR_EPOCH_PLAYER_135"),_needText,_distance] , "PLAIN DOWN", 3]; };
...
// Message If missing
if (!_hasRequiredTools) exitWith {DZE_ActionInProgress = false; titleText [format[(localize "str_epoch_player_137"),_missingT] , "PLAIN DOWN", 3];};

if (!_typeIsString && !_hasbuilditem) exitWith {DZE_ActionInProgress = false; titleText [format[(localize "str_epoch_player_146"),_missingQty, _missingB], "PLAIN DOWN", 3];};

if (_typeIsString && !_hasbuilditem) exitWith {DZE_ActionInProgress = false; titleText [format[(localize "str_player_31"),_text,"build"] , "PLAIN DOWN", 3]; };
...
if (_hasRequiredTools && _hasbuilditem) then {
	...
	titleText [(localize "str_epoch_player_45"), "PLAIN DOWN", 3];
	...
	if(!_cancel) then {
		...
		titleText [format[(localize "str_epoch_player_138"),_text], "PLAIN DOWN", 3];
		...
		while {_isOk && !_isBuildAdmin} do {
			...
			titleText [format[(localize "str_epoch_player_139"),_text, _counter,_limit], "PLAIN DOWN", 3];
		...
		//diag_log "Proceed OK";
			...
			if((_tobe_removed_total == _removed_total) && (_removed_total >= 1)) then {
				...
				titleText [format[localize "str_build_01",_text], "PLAIN DOWN", 3];
				...
				if(_lockable > 1) then {
					...
					titleText [format[(localize "str_epoch_player_140"),_combinationDisplay,_text], "PLAIN DOWN", 5];
				...
				titleText [(localize "str_epoch_player_46") , "PLAIN DOWN", 3];
				{
		...
		deleteVehicle _object;
		titleText [format[(localize "str_epoch_player_47"),_text,_reason], "PLAIN DOWN", 3];
	};
};
